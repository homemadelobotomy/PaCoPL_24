...
def main():
    ...
    print("\nЗадание Д2\n«Каталог» и «Файл» связаны соотношением один-ко-многим.Выведите список каталогов со средним весом файлов в каждом каталоге, отсортированный по среднему всеу.")

    res_2_not_sorted = []
    #Проходим серез все каталоги и спсиок один ко многим. проверяем совпадение каталога файла, после чего добавляем в несортированный словарь результата средний вес файлов в каталоге.
    ...
    print("\nЗадание Д3\n")
    print("«Каталог» и «Файл» связаны соотношением многие-ко-многим. Выведите список всех каталогов, у которых название начинается с буквы «Д», и список фалйов, находящихся в них.")

    res_3 = {}
    #Находим каталоги, которые начинаются на 'Д' и через список многие ко многим находим все фалы которые находятся в этом катологе и добавляем в словарь результата, где ключ - Название каталога, а значение - список файлов
    for cat in catalogs:
        if cat.name[0] == 'Д':
            cat_files = []
